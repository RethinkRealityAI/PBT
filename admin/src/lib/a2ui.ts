 ...
 *
 * Ported from PhotoBoothAR (src/lib/a2ui.ts, Apache-2.0) for the Scenario
 * Studio assistant. A2UI is Google's open protocol for agent-driven
 * interfaces: a stream of declarative JSON messages (createSurface /
 * updateComponents / updateDataModel / deleteSurface) describing UI *intent*
 * as a FLAT list of components with id references plus a JSON data model;
 * the client renders them with its own trusted component catalog and reports
 * interactions back as `action` events. "Safe like data, expressive like
 * code."
 *
 * In PBT the model never authors A2UI directly: trusted builders
 * (admin/src/scenario-studio/copilot/agentSurfaces.ts) turn NORMALISED
 * assistant proposals into surfaces. This module is the pure half — types,
 * JSON-Pointer data model (RFC 6901), the surface reducer, and binding
 * resolution — with no React/DOM imports so it runs under any test env.
 */
...
/** PBT's custom catalog: the basic catalog PLUS Studio widgets the renderer
 *  trusts (A2UI extension mechanism — surfaces may only use registered
 *  components). See the renderer for the list. */
export const PBT_STUDIO_CATALOG_ID = 'https://pbt.rethinkreality.ai/a2ui/catalogs/studio-v1';
 ...
 * to null rather than throwing — an agent typo must never crash the Studio.
 */
