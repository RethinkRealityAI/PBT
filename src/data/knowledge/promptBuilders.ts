...
export function buildVoiceSystemPrompt(scenario: Scenario): string {
  ...
  return (
    ...
    `- LEAD-IN WORD: ALWAYS begin every spoken turn with one short verbal lead-in word — "Well," / "Look," / "Okay," / "Hmm," / "So," / "I mean," — followed by a brief pause, then your actual content. This is critical: the speech-to-text frequently drops the first few hundred milliseconds of audio, and a one-word lead-in means it loses the filler instead of substance. Vary the lead-in across turns; never start a turn directly with the substantive first word.\n` +
